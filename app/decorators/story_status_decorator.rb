...
class StoryStatusDecorator < AgileBoardDecorator
  ...
  def edit_link(project, path = nil)
    super(project, h.agile_board_plugin::edit_project_story_status_path(project.slug, model.id))
  end

  def delete_link(project, path = nil)
    super(project, h.agile_board_plugin::project_story_status_path(project.slug, model.id))
  end
